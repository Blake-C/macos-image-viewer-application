...
struct FullImageView: View {
	...
	@ViewBuilder
	private var imageZStack: some View {
		ZStack {
				...
				HStack(spacing: 8) {
					...
					Button {
						withAnimation(.easeInOut(duration: 0.2)) {
							state.showMetadataPanel.toggle()
						}
					} label: {
						Image(systemName: state.showMetadataPanel
		...
		.contextMenu {
			...
			Button {
				...
				Label(
					state.isFavorite(currentURL) ? "Remove from Favorites" : "Add to Favorites",
		...
		.task(id: currentURL) {
			...
			// Load image, info, pixel size and full metadata concurrently
			...
			async let meta      = ImageLoader.rawMetadata(for: currentURL)
			let (loadedImg, loadedInfo, loadedPixelSize, loadedMeta) = await (img, info, pixelSize, meta)

			imageInfo   = loadedInfo
			state.currentImageMetadata  = loadedMeta
